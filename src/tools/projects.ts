...
import { Project, BriefingResult, Task, ActiveSession } from '../types';
import { getVaultManager } from '../vault/multi';
import { readMarkdownFile, listMarkdownFiles } from '../vault/reader';
 ...
 */
async function findProject(projectsPath: string, name: string): Promise<string | null> {
  let files: string[];
  try {
    files = await listMarkdownFiles(projectsPath);
  } catch {
    return null;
  }
 ...
 */
function extractOpenTasks(body: string, filePath: string): Task[] {
  const tasks: Task[] = [];
  ...
  lines.forEach((line, index) => {
    ...
    if (match) {
      ...
      if (/⏫/.test(text)) priority = 'high';
      else if (/🔼/.test(text)) priority = 'medium';
      else if (/🔽/.test(text)) priority = 'low';
 ...
 */
function extractLastSessionSummary(body: string): string | undefined {
 ...
 */
function formatBriefing(briefing: BriefingResult): string {
 ...
 */
export async function getBriefing(input: Record<string, unknown>): Promise<ToolResult> {
  ...
  const projectsPath = path.join(vaultConfig.path, structure.projectsFolder);
 ...
 */
export async function listProjects(input: Record<string, unknown>): Promise<ToolResult> {
  const vaultName = input['vault'] as string | undefined;
  const statusFilter = (input['status'] as string | undefined)?.toLowerCase();
 ...
 */
export async function focusProject(input: Record<string, unknown>): Promise<ToolResult> {
  const projectName = input['project'] as string | undefined;
  const vaultName = input['vault'] as string | undefined;
