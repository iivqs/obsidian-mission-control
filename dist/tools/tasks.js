...
function parseTaskLine(line, filePath, lineNumber) {
    ...
    return {
        id: `${path_1.default.basename(filePath, '.md')}-L${lineNumber}`,
        text,
        completed,
        due,
        priority,
...
// ---------------------------------------------------------------------------
function parseTasksFromBody(body, filePath) {
    const tasks = [];
...
async function getTasks(input) {
    ...
    try {
        ...
        if (projectName) {
            ...
            const projectsDir = path_1.default.join(structure.root, structure.projectsFolder);
            const projectFiles = await (0, reader_1.listMarkdownFiles)(projectsDir);
            const match = projectFiles.find((f) => path_1.default.basename(f, '.md').toLowerCase() === projectName.toLowerCase());
            if (match)
                projectFilePath = match;
        }
        else {
        ...
        if (projectFilePath && (await (0, reader_1.fileExists)(projectFilePath))) {
            const { body } = await (0, reader_1.readMarkdownFile)(projectFilePath);
        ...
        if (structure.tasksFolder) {
            const tasksDir = path_1.default.join(structure.root, structure.tasksFolder);
            const taskFiles = await (0, reader_1.listMarkdownFiles)(tasksDir, false);
            for (const filePath of taskFiles) {
                try {
                    const { body } = await (0, reader_1.readMarkdownFile)(filePath);
...
async function updateTask(input) {
    ...
    try {
        ...
        const session = (0, session_1.getActiveSession)();
        if (session) {
            const projBase = path_1.default.basename(session.project.filePath, '.md');
        ...
        for (const f of projectFiles) {
            if (path_1.default.basename(f, '.md') === basename)
                candidatePaths.push(f);
        }
        // Check Tasks/ folder
        if (structure.tasksFolder) {
            const tasksDir = path_1.default.join(structure.root, structure.tasksFolder);
            const taskFiles = await (0, reader_1.listMarkdownFiles)(tasksDir);
        ...
        if (!/^(\s*)-\s+\[[ x/]\]/.test(targetLine)) {
            return {
                content: [
                    {
                        type: 'text',
                        text: `Error: line ${lineNumber} in "${path_1.default.basename(filePath)}" does not look like a task: ${targetLine}`,
                    },
                ],
            };
        }
...
async function logDecision(input) {
    ...
    try {
        ...
        if (session) {
            ...
            const sessionsDir = path_1.default.join(structure.root, structure.sessionsFolder);
            // List session notes and look for today's date + optional project match
            const sessionFiles = await (0, reader_1.listMarkdownFiles)(sessionsDir);
            const candidates = sessionFiles.filter((f) => {
                ...
                if (!hasToday)
                    return false;
                if (projectName)
                    return base.toLowerCase().includes(projectName.toLowerCase());
                return true;
            });
        ...
        if (!notePath) {
            return {
                content: [
                    {
                        type: 'text',
                        text: 'Error: no active session and no session note found for today. ' +
        ...
        await (0, reader_1.appendToFile)(notePath, entry);
        return {
        ...
        return {
            content: [{ type: 'text', text: `Error logging decision: ${err.message}` }],
        };
    }
}
